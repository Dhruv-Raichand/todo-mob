export const COLORS = {
  // Primary Blues (Professional)
  primary: '#1E3A8A', // Deep Blue
  primaryLight: '#3B82F6', // Bright Blue
  primaryDark: '#1E40AF', // Navy Blue

  // Secondary Accent
  secondary: '#10B981', // Professional Green
  accent: '#6366F1', // Indigo

  // Backgrounds
  background: '#F8FAFC', // Light Gray-Blue
  surface: '#FFFFFF', // White
  card: '#FFFFFF', // White Cards

  // Status Colors (Professional)
  success: '#10B981', // Green
  warning: '#F59E0B', // Amber
  error: '#EF4444', // Red
  info: '#3B82F6', // Blue

  // Text Colors
  text: '#1E293B', // Dark Gray-Blue
  textSecondary: '#64748B', // Medium Gray
  textLight: '#94A3B8', // Light Gray

  // Borders & Dividers
  border: '#E2E8F0', // Light Border
  divider: '#CBD5E1', // Divider

  // Disabled States
  disabled: '#CBD5E1', // Gray
  disabledText: '#94A3B8', // Light Gray

  // Overlay
  overlay: 'rgba(15, 23, 42, 0.5)', // Dark overlay

  // Priority Colors (Professional)
  priorityLow: '#10B981', // Green
  priorityMedium: '#3B82F6', // Blue
  priorityHigh: '#F59E0B', // Amber
  priorityUrgent: '#EF4444', // Red

  // NEW: Deadline Urgency Colors
  deadlineNormal: '#10B981', // Green (7+ days)
  deadlineWarning: '#F59E0B', // Amber (4-7 days)
  deadlineUrgent: '#FB923C', // Orange (2-3 days)
  deadlineCritical: '#DC2626', // Dark Red (1 day)
  deadlineOverdue: '#991B1B', // Very Dark Red (overdue)
};
